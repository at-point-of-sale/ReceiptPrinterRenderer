...
Text is drawn with a built in bitmap font, [Iosevka](https://github.com/be5invis/Iosevka) Medium under the SIL Open Font License 1.1, in the 12 by 24 cell of font A and the 8 by 16 cell of font B, with three subsets behind it for the scripts it has no glyph for, each under the same licence: [Sarasa Gothic](https://github.com/be5invis/Sarasa-Gothic) Mono J SemiBold for the half width katakana, [Noto Sans Hebrew](https://github.com/notofonts/hebrew) Medium for the Hebrew and [Noto Sans Thai](https://github.com/notofonts/thai) Medium for the Thai. A glyph comes from the first of the four that has the character, and all four are drawn at the scale of Iosevka, which is the face.

The fonts are made from those faces in [ReceiptPrinterFontEditor](https://github.com/at-point-of-sale/ReceiptPrinterFontEditor), where the glyphs the rules get wrong at this size are drawn by hand on the dot grid, and this package carries what it exports. The box drawing and block characters are not taken from a face at all: they are drawn by a rule on the dot grid, so that the lines of a box or a rule join across the cells, with double lines two dots thick and rounded corners as arcs in the SVG output.
