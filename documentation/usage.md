...
The built in font is [Iosevka](https://github.com/be5invis/Iosevka) Medium, under the SIL Open Font License 1.1, with a subset of [Sarasa Gothic](https://github.com/be5invis/Sarasa-Gothic) Mono J SemiBold behind it for the half width katakana, rasterized into a 12 by 24 cell for font A and an 8 by 16 cell for font B when the package is built. The box drawing and block characters are drawn on the dot grid, so that the lines of a box or a rule join across the cells. Every code point of cp437, of the ISO 8859 and Windows Latin codepages, of the Greek and Cyrillic ones, and of the katakana page of either printer family, the twelve kanji of Epson's table included, has a glyph; a character without one, which is Arabic, Khmer, Thai and Hebrew, is printed as U+FFFD.
