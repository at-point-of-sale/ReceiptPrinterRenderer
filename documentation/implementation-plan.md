...
- `Font.renderGlyph` centres the glyph in the cell, horizontally and vertically, so that an 8x16 glyph sits in the middle of a 9x24 cell. **Corrected on 2026-09-13**: the glyph is centred horizontally and stands on the baseline of the cell vertically, `font.cellBaseline(cellHeight) - baseline` dots from the top, so an 8x16 glyph starts on row 6 of a 9x24 cell and on row 0 of a 9x17 one. Bold overstrikes before scaling, as a printer does, the underline is drawn after scaling and stays one or two dots thick, and inverting is last. An inverted cell is not underlined, which is what the ESC/POS reference of `ESC - n` says.
...
- **Cells sit at the top of the line box**, the gap of the line spacing falls below them. A consequence is that the vertical bars of the box drawing characters span their cell only, so a box has a six dot gap at every line boundary with the Epson line spacing of 30 dots and a 24 dot cell, exactly as the design's Painter section describes and as an Epson prints. Horizontal joins are seamless, because `stretch` pulls the ink out to the edges of the cell. **Corrected on 2026-09-13**, confirmed on an Epson printout: a cell stands on the baseline of its line, the bottom edge of the tallest cell on it, at `tallest - cell` dots from the top of the line box, so a single height character next to a double height one sits on the same line as the tall one and the underline of `_w/_` in receiptline's text decoration document runs along that line. Only the gap of the line spacing stays below the cells, the line height and the advance are the ones above, and a line whose cells all have one height is unchanged by a single dot, which is why only the fixtures that mix sizes on a line moved. See the Baseline bullet of the design's Painter section and `#commit()` in `src/painter.js`.

  **Corrected again on 2026-09-13**, on the same Epson printout read more closely: cells of different sizes do not stand on the bottom edge of the tallest cell, they share the baseline of the font. A font carries the row its glyphs stand on, row 18 of the 24 row font A cell and row 12 of the 16 row font B one, and a cell of any height has its baseline at the same fraction of that height, which is what `Font.cellBaseline(cellHeight)` returns. The ascent of a text cell is that row times the height multiplier and its descent is the rest of the cell; the line is as tall as the largest ascent plus the largest descent, and a cell is drawn `ascentMax - ascent` dots from the top of it. On the burger line of receiptline's text decoration document the small `w/`, `tomato,` and `mayonnaise` now sit on the same baseline row as the double size `meat sauce`, with the descender space of the tall cells below them, which is what the paper shows. Strips of a column mode image and cells turned by `ESC V` have no baseline and keep sitting on the bottom of the line box, so a strip taller than the text pushes the text down. A line whose cells all have one font and one size is still unchanged to the dot, so only the fixtures that mix sizes on a line moved, plus the Star fixtures that use font B, whose 8x16 glyph now starts on row 6 of its 9x24 cell instead of row 4. Nothing of the history above is deleted, the rule of this paragraph is the one that holds.
