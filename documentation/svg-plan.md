...
- **The outlines are bundled with the writer and never published on their own.** `data/fonts/outlines.js` is imported by the SVG entry alone. There is no peer dependency, no second copy in a second repository, and no `@point-of-sale/receipt-printer-renderer/outlines` entry.
...
- The rasterized check in `test/svg.js`: every fixture of the golden directories and the raw directories rendered by `@resvg/resvg-wasm`, a dev dependency, at one dot per unit, thresholded at half, and compared with the golden PBM of the fixture. The regions the layout's rectangles and images cover agree dot for dot; the whole paper agrees to at least 0.98 by the measure of `tools/contact-sheet/references/shared.js`, with the number per fixture in the notes, since an outline filled by resvg and a bitmap made with the 0.45 rule differ at the edges of a stroke. *Amended 2026-09-15: the bound is 0.96. The built in font is tweaked by hand on the dot grid now, which the Decisions list of this document asks for, so the bitmaps are no longer the outlines of the face rasterized and the two disagree along a stroke by more than the coverage rules alone put them apart.*
- `tools/contact-sheet/references/svg.js`: a reference column that renders the SVG of every external fixture through resvg, so the contact sheet shows the vector output next to the bitmap with the agreement number, and the eye review of Section 16 extends to it. *Amended 2026-09-15: the page shows the SVG documents themselves and not the raster; a raster at one pixel per dot is a bitmap again and showed nothing of the vector output. The list is split at its cuts with `pieces()`, a new helper of the main entry, and every piece is written as a document of its own, one per piece of paper like the other columns. resvg still rasterizes the whole list at one pixel per dot for the agreement number, and that raster is written next to the documents as `.svg.png`.*
...
- Path precision is tenths of a dot for the outlines and whole dots for the box glyphs and the traced bitmaps. *Amended 2026-09-16: the box glyphs are geometry since the editor's Section 21, in dots with at most three decimals, since the arms of a rounded corner are cut at a half dot and its arcs land where they land.*
...
*Amended 2026-09-16: the clip of Section 4 is gone for the glyphs of the face and stays for a downloaded glyph. The font of today is fitted by the editor without a squeeze for font A and its tall glyphs are finished by hand on the dot grid, so the outline of a brace reaches two dots above its cell and the clip drew it with its top cut flat. Squeezing the outline into the cell instead was tried and rejected the same day: the bitmap font is drawn by hand precisely so that no glyph is squeezed, and the vector output has the face and no cell to answer to. A glyph of the face is drawn where it lies, above its cell when it reaches above it. What leaves a cell today leaves it at the top, or by a dot at the right for the horned letters, so nothing runs into a neighbour.*
...
*Amended 2026-09-16: the box set is geometry and not a trace of the dots, since Section 21 of ReceiptPrinterFontEditor's plan. The editor draws U+2500 to U+259F from one description per glyph and cell, fills it onto the dot grid for the packed font and writes the same description here as path data in dots, so the two are the same shape and neither follows the other: a hand edit inside the range moves the printed dots and leaves the path where the rule puts it. An arm, a band, a block and a dot of a shade are still rectangles on whole dots in the tracer's `M x y h w v h h -w Z` syntax, and the four rounded corners U+256D to U+2570 are not: each is a subpath of an outer arc, a line across the stroke, the inner arc back and a close, written with the SVG `A` command, on fractional coordinates of at most three decimals. A double line is two two dot strokes a two dot gap apart where it used to be two one dot strokes, and a single line that ends at a double that passes through stops at the near line of it. The 9x17 and the 9x24 sets are the 8 by 16 shapes placed and stretched the way `Font#renderGlyph({stretch: true})` places and stretches the dots, so the inheritance this paragraph wanted from tracing the rendered cell is worked out in the editor instead. `src/svg/writer.js` draws whatever path the set holds and needed no change; the `box` paragraph of `design.md` and the command set below say it the new way.*

**The command set, exactly.** A glyph path is absolute `M`, `L`, `Q`, `C` and `Z`; a `C` stands only where a curve of the face is not a quadratic one to within a tenth of a dot, which this face never is, so what is in the file today is `M`, `L`, `Q` and `Z`. A box path is `M`, `Z` and the relative `h` and `v` of the rectangles, in dots, and, since the amendment above, the absolute `A` and `L` of the arcs of a rounded corner. The format note of the generator, the paragraph of `design.md` and the format block of this section say it that way, and the block's box example is a path out of the file. A glyph may also be the empty string, which the carriage return, the space and the no break space are, so a consumer asks `codepoint in glyphs` and does not test the string.
...
Date: 2026-09-13. `src/svg.js`, `src/svg-umd.js`, `src/svg/writer.js` and `src/svg/png.js` added, `src/formats/png.js` split, the build, the `exports` map and `tsconfig.json` extended, `test/svg.js` and `tools/contact-sheet/references/svg.js` added, sixteen golden SVG files frozen.

- **The version stays 0.3.0.** The plan makes this section 0.4.0; the maintainer decided otherwise while it was being implemented, so `package.json` is untouched but for the dev dependency. The release is a decision of its own.

**The files.** `src/svg.js` is the sub-entry, `toSvg` as the default and a named export; `src/svg-umd.js` is the entry of its UMD build, the named exports alone, for the same reason `src/umd.js` exists; `src/svg/writer.js` is the writer, 700 lines, one class that collects the definitions and the clip paths while it writes the body and assembles the document afterwards; `src/svg/png.js` is the PNG of stored deflate blocks; `src/svg/trace.js` is Section 3's tracer, unchanged. `src/formats/png.js` gained `scanlines()`, which is the rows of a bitmap with their filter bytes, and `encode()`, which is the signature, the header, an IDAT of whatever deflate stream it is given and the end marker, and exports `SIGNATURE`, `crc32()` and `chunk()` next to them. `toPng()` is now `encode(bitmap, await deflate(scanlines(bitmap)))` and the stored writer is the same line without the `await`, so the two files are identical up to the deflate stream, which is what the section asked for. `test/formats.js` passes unchanged, so `toPng()` is byte for byte what it was.

**The elements of a cell, and the clip that was measured.** A cell that is not turned is written in the coordinates of its line, with no group at all: the inversion rectangle, the glyph, the overstrike and the lines are four elements that each carry their own position. A cell turned by `ESC V` is the same four pieces in the frame of the unturned cell inside one `<g transform="translate(x+h0 y)rotate(90)">`. The clip of a glyph is written on the glyph element itself and not on a group around it, which is the first of the two forms the section asked to measure: `clip-path` on an element is in the coordinate system of that element, after its own transform, which resvg and the browsers agree on, and in that system the cell is the rectangle `(-left / fit, -top / fit, cell.width / fit, cell.height / fit)`, which depends on the cell and the glyph box and not on the size the cell is drawn at. One clip path therefore serves every cell of a shape at every scale, and the `receipt` fixture has three in all: `paper`, the one every line is clipped to, `k0_0_12_24`, the 12 by 24 cell of font A, which the box drawing characters of a cell of that shape share because their box is the cell, and `km1_0_12_24`, the same cell for the overstrike one dot to the right. `fonts`, which prints both fonts and box drawing characters in the 9 by 17 cell, has four. Measured against the other form, the same document with every glyph nested in a `<g transform clip-path>`: `receipt` 37,460 bytes against 38,120, `styles` 14,675 against 15,021, `box` 25,269 against 26,079. The clip on the element is 1.8 to 3.2 per cent smaller and it is also the form that needs no group for the ordinary cell, so it is the one that is written.

**Where the glyph sits.** `left` is `floor((cell.width - glyph.width) / 2)` and `top` is `operation.baseline - baseline of the font`, which is `renderGlyph()`'s placing with the numbers of the list. The baseline of the font is the `baseline` of the outlines, 18, for font A and two thirds of it, 12, for the 8 by 16 box of font B, which is the baseline of the packed 8x16 font; a `glyphsB` entry, when the generator ever writes one, is in the 8 by 16 box and stands on the same 12. A downloaded glyph has the cell as its box and the baseline of the cell, so it lands in the corner, which is where the back-end blits it.

**Decisions where the plan was silent.**

- **The ink is a `fill` on the `<svg>` element**, which every shape inherits, so a cell costs no colour of its own. The glyph of an inverted cell carries the background colour, white when the paper is transparent, and nothing else does.
- **The rectangles that stand next to each other in a line become one path**, not every rectangle of the line: the run is flushed when a cell or an image comes between them, so the order of the list is kept exactly, which matters for an inverted cell that paints white. A barcode, a QR code, a PDF417 symbol and a DataBar are one run and so one element each.
- **A line, an area or a page that holds nothing is not written at all**, which keeps a receipt of ten lines to ten groups where the list carries the feeds between them as entries of their own.
- **A line is clipped to the width of its surface and not to its own height.** The bitmap back-end composes a line into a bitmap of `entry.height` rows, so it cuts an operation that is taller than its line box; the document cuts it at the width of the paper and lets it hang below the line. The engine never emits such an operation, the height of a line box is the tallest cell on it, so nothing in a list `layout()` returns reaches it. A hand-built line of 20 rows holding a 48 dot cell does: measured against `rasterize()` of the same list, a list of 60 rows with that line paints 178 dots the back-end does not, the part of the glyph that hangs below the line box, and the back-end's paper is 20 rows where the document is 60. A consumer may rely on the rule, a hand-built list has to respect it, and `display-list.md` says so under the `line` entry. A clip per line would cost an element on every line of every receipt for a case the format rules out. *Amended 2026-09-16: the paper clip that every line carried was applied in the line's own frame, since a clip on a transformed element lives in that element's user space, so it did cut a line at its own top after all, which showed once the glyphs were no longer clipped to their cells: the top of a brace on any line but the first was flat. The clip is now one group around the whole body, in the frame of the paper, and a line carries none.*
- **A line inside a print area is not clipped separately.** The area is clipped to the intersection of the area and the page, and the logical surface of an area maps exactly onto that box, so the area clip cuts a line at the same place the line bitmap of the back-end does.
- **The area is two groups**, the clip on the outer one and the transform of the direction on the inner one, so that the clip is read in the frame of the page and not in the turned frame of the area.
- **The box set is keyed by the cell**, `x2500-9x17`, and used when the font of the operation is the font that set belongs to, A for the 12 by 24 cell and B for the 9 by 17 and 9 by 24 ones. The set is geometry rather than a trace since 2026-09-16, see the amendment in Section 3, and the writer draws whatever path it holds, arcs and all. A cell of another shape, or one of these in the other font, is traced at write time from the bitmap font with `stretch: true`, cached per code point, font and cell, and defined under `t2502-A-10x20`. That is the only thing the writer needs `src/font.js` for, and it is what a profile of an application's own reaches.
- **A downloaded glyph is written where it is used**, as the plan says, so a stream that defines one glyph and prints it fifty times writes fifty paths. It is the one thing in the document that is not shared, and no fixture makes it hurt: `user-defined` is 23 kB with six of them.
- **An unknown code point is the fallback glyph**, U+FFFD of the face, under the same id as any other glyph, `afffd`. A code point whose outline is the empty string draws nothing while its cell is still inverted, underlined and upperlined, and the writer asks `codepoint in glyphs` rather than testing the string.
- **The golden files are written with the defaults of the writer**, so `cut.svg` shows that a cut is nothing without `cutMarker`; the marker itself is checked element by element in `test/svg.js`. `page-mode-directions` is frozen for StarPRNT, which is the language its golden display list is frozen for.
- **An image is the PNG `toPng()` writes**, one bit greyscale, black on white. It therefore does not take the `ink` colour and it paints white paper under itself on a transparent background. A palette with a `tRNS` chunk would fix both and would no longer be the file `toPng()` writes; the section asks for the file `toPng()` writes.
- **Two places paint white where the paper only ever adds ink**, which is the one way the document can differ from the dots by more than the edge of a stroke, and both need a reverse feed to reach: an inverted cell that a later entry puts over a printed line writes its glyph in the colour of the paper, and an image writes its white dots as white, so a raster image that a reverse feed puts over printed text erases it in the document and overprints it on the paper. The bitmap back-end combines with OR and cannot do either. No fixture of the repository does it, and a repro of each was built by hand and measured against `rasterize()` of the same list: a 48 by 24 image over four cells of `A` erases all 344 dots of them in the document and overprints them on the paper, and an inverted space over two cells erases 6 dots of their stems, the rest of the cell being white in both. Both are the same class of deviation and both are written down here, in `design.md` and in the notes rather than worked around: keeping them would cost a mask per image and a second pass over the earlier ink.
- **The cut marker** is a one dot line with `stroke-dasharray="12 8"` at the row of the cut, in the ink colour. The dashes are a choice, not a number from anywhere.
- **A list of no height is a document of one blank row.** A stream that printed nothing lays out to `height` 0, and `height="0" viewBox="0 0 576 0"` is refused by resvg, "SVG has an invalid size", and drawn as nothing by a browser. It is the one place the document is not the paper of the list, and the only one: the width is never 0, a paper is a multiple of eight dots. The `toSvg()` JSDoc, `usage.md` and `display-list.md` say so.
- **`units`** is worked out from the `dpi` of the list: `mm` is `25.4 / dpi` per dot, `pt` is `72 / dpi`, `px` is `96 / dpi`, so the 576 dots of an Epson at 203 dpi are 72.070936 mm. Unknown units throw, the way an unknown version does. Numbers are written with at most six decimals and without the zero before the point, `scale(.1)`.

**The sizes.** The `receipt` fixture is 37,460 bytes, 7,997 gzipped, of which 19,086 bytes is the definitions of the 43 distinct glyphs it uses, 444 bytes each; the rest is 183 glyph references, 196 rectangles in two paths and ten groups. That is well inside the 60 kB of the acceptance. Of the text only fixtures, `styles` is 14,675 bytes with 16 glyphs, `cut` is 10,662 with 16 and `sizes` is 8,767 with 12, all inside the 20 kB, but `text` is 29,715 bytes with 49 distinct glyphs and `fonts` is 19,467 with 34: a fixture that prints every accented character of a codepage carries a definition per character and the 20 kB is a bound on the number of distinct glyphs, 45 of them at 444 bytes, and not on the length of a receipt. A real receipt of a hundred lines in one alphabet stays where `receipt` is. The largest golden file is `star-graphics` at 73,255 bytes, which is two embedded images of a whole receipt as dots and no text at all, 4,927 gzipped.

The builds of the sub-entry: `receipt-printer-renderer-svg.umd.js` 437,628 bytes and 105,568 gzipped, `.esm.js` 437,395 and 105,340, `.mjs` 499,038 and 120,140, `.cjs` 499,129 and 120,243, `.d.ts` 2,478. The main UMD build is 225,094 bytes against the 225,040 of Section 2, 54 bytes more, which is the split of `png.js`. The sub-entry is larger than the 40 kB the risk list of this document guessed, for the reason the notes of Section 3 already gave, 344 kB of outlines rather than 150, and it carries the packed fonts on top of them, 82 kB of source, for the one case that traces a cell at write time. Dropping that case would take about a fifth off the gzipped bundle and would leave a profile of an unusual cell size without box drawing characters; the section asks for the case, so it is in.

**The rasterized check.** `@resvg/resvg-wasm` 2.6.2, pinned exactly, a dev dependency of `test/svg.js` and of the contact sheet column. It loads in Node 22.19 and renders every fixture in a few milliseconds, so the whole check is under a second. The document is rendered at its original size, one dot per unit, and read back with the PNG reader of `tools/contact-sheet/references/shared.js` at its threshold of 128, which is half.

All 101 fixtures of the four golden and raw directories pass: the rectangles and the images agree dot for dot, every one of them, and the whole paper agrees to at least 0.98. Lowest `esc-pos/raw/spacing` 0.98280, then `esc-pos/raw/rotation` 0.98479, `esc-pos/raw/print-mode` 0.99140, `star-prnt/raw/print-mode` 0.99179, `esc-pos/wrap` 0.99246; median 0.99842; 14 fixtures agree exactly, which are the ones that carry no text. The test prints the number of every fixture. The differences are single rows at the top or the bottom of a stroke, where the 0.45 coverage rule of the rasterizer and the half coverage of resvg disagree, and they are worst where a fixture is small and its text is large: `spacing` is 138 rows of double height characters, so 1,104 dots is a dot row of a stem here and there.

*Amended 2026-09-15: `generated/fonts.js` was re-exported from the font editor, with 818 of the 994 glyphs of font A and 91 of font B tweaked by hand on the dot grid, the baselines and the cell sizes unchanged. That is the drift from the face the Decisions list asks for, and it is the whole of the change here: the outlines are still the curves of Iosevka, so a hand tweaked stem now differs from its outline along its length and not only at the row where the two coverage rules disagree. The bound is 0.96. All 109 fixtures of the four golden and raw directories pass, the rectangles and the images still dot for dot. Lowest `esc-pos/wrap` 0.97263, then `star-prnt/wrap` 0.97434, `esc-pos/raw/spacing` 0.97591, `esc-pos/raw/rotation` 0.97740 and `esc-pos/raw/katakana-kanji` 0.97784; median 0.99387; the same 14 fixtures agree exactly, which are the ones that carry no text. The five lowest are the fixtures whose papers are small and whose text is dense or large: the two `wrap` fixtures are a paragraph on ninety rows of paper, `spacing` is 138 rows of double height characters, `rotation` turns its cells a quarter turn, and `katakana-kanji` prints the kanji of the katakana table, which the face draws on a full em and the font fits into half of one.*

The 147 external fixtures, measured by the contact sheet column, run from 97.18 to 100 per cent with a median of 99.56. The lowest are the ones that print the largest text, `escpos-php/text-size` at 97.18 and `python-escpos/receipt` at 98.25: a character at eight times the size has a staircase of eight dots on the paper against a smooth curve in the document, which is the point of the vector output rather than a fault of it.

**The measure lives in `shared.js`.** The section asks for the measure of `tools/contact-sheet/references/shared.js`, which had none of this kind: `agreement()` there is the coarse row metric of Section 16b, for a reference renderer of another width. `dotAgreement(ours, other)` was added next to it, the fraction of the dots of the larger of the two papers that are the same in both, and `test/svg.js` and the new reference column both import it.

**The eye review.** The `receipt`, `styles`, `box`, `fonts`, `rotation`, `user-defined` and `page-mode-directions` golden files were rendered with resvg and read next to the PNG of the same paper. They read as the same receipt in every case, down to the dot: the bold total, the inverted cell with its white glyph, the underline under a word, the box drawing characters closing their boxes in both fonts, the quarter turn of `ESC V` with its cells running left to right, the four print directions of a page in the corners the display list maps them to, and the six downloaded glyphs in the middle of a line of text. Nothing had to be fixed before the files were frozen. The one visible difference is the one the section predicted: the outlines are smooth where the dots are square, which the external `text-size` fixture shows at its clearest, a `Hello world!` at eight times the size that is a staircase on the paper and a curve in the document.

**Two smaller things.** `test/svg.js` writes its own XML parser, a hundred lines that know elements and quoted attributes and throw on anything else, which is what makes "is well formed" a check rather than a regular expression, and its own transform evaluator, so that the four print directions are checked by applying the transform of an area to a point and comparing it with the mapping of the contract rather than by matching the text of the attribute. And resvg is loaded at the top of that file rather than in a `before` hook: the Google eslint config forbids `this` in a hook, which is how mocha's timeout and skip are reached, and a top level await lets the file define the tests only when the module is there.

**The dependency was added by hand.** `node_modules` of this checkout holds four linked local packages, one of which, `@point-of-sale/star-graphics-printer-encoder`, is not published, so `npm install` of anything resolves the whole tree and fails on it. `@resvg/resvg-wasm@2.6.2` was fetched with `npm pack`, unpacked into `node_modules/@resvg/resvg-wasm`, and written into `package.json` and `package-lock.json` by hand with the integrity `npm view` reports. A checkout without the links installs it the ordinary way.

**And `npm ci` does not run on a clean checkout, for a reason older than this section.** The root `devDependencies` of `package-lock.json` do not carry the four `@point-of-sale` driver packages that `package.json` lists, the encoder, the Star graphics encoder and the three printer drivers, because they are linked locally and the lock has not been written since they were added; `npm ci` refuses a lock that does not match the manifest. The lock has to be refreshed with a plain `npm install` once those packages are published, and the entry this section added, which is an ordinary registry dependency, survives that untouched.

**A follow-up: the sub-entry carries the packed fonts for one case.** `data/fonts/fonts.js` is in the bundle, about a fifth of its gzipped size, and the writer reaches it only to trace a box drawing character in a cell size the box set of the outlines has no entry for, which is a profile of an application's own. Two ways to drop it were assessed and neither is this section's work. The first is to split the packed table out of `src/font.js`, so that the writer imports the renderer of a glyph and not the data, and to take the packed font as an option of `toSvg()`, which an application that has its own profile has anyway; the entry then has no font at all and an unusual cell without the option falls back to the box path of the nearest cell. The second is to skip the trace and scale the box path of the nearest cell non-uniformly, which needs no font and thickens the strokes of a bar by the ratio of the cells, so a box drawn at 10 by 20 from the 12 by 24 set has bars of 1.7 dots. The first is the better one and costs a public option; it is left for whoever wants the sub-entry smaller.
