...
- **The body** is one `<g>` clipped to the paper, in the frame of the paper, holding every line and page: an operation may run past the right edge of its surface and the line bitmap of the renderer clips it, and a glyph of the face may reach above its line box, which the paper does not cut. The clip is not on the lines themselves, because a clip on an element with a transform is applied in that element's frame and would cut a glyph at the top of its line box.
- **A line** is `translate(0 y)`, or `translate(W y+h) rotate(180)` when it was printed upside down, and carries no clip of its own.
