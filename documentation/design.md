...
A glyph path of `glyphs` and `glyphsB` is absolute M, L, Q, C and Z, in the frame of the cell with y pointing down, in units of a tenth of a dot, so it is placed under `scale(.1)`. A C stands only where a curve of the face is not a quadratic one to within a tenth of a dot; there is none in the face of today, so these paths are M, L, Q and Z. A box path is M, Z and the relative h and v of the rectangles a trace of the rendered cell emits, in whole dots, and nothing else. The command letter is left out where it repeats, in both. The fill rule is nonzero, the rule TrueType is drawn with.
...
A path is not clipped by its cell and the bitmap of a glyph is. 43 of the glyphs of the committed font paint outside the 12 by 24 cell, none of them by more than two dots and none of them wholly; a consumer clips every glyph to its cell, which is what the bitmap does for the printer, and `test/outlines.js` counts them and reports the number rather than pinning it. What leaves the cell leaves it at the top and not at the side, the Vietnamese precomposed letters and the horn letters of a face drawn for a taller cell.

A glyph path is the same set of placed contours the bitmap of the glyph was filled from, so an outline sits on the bitmap it was rasterized from, with one exception in each direction. **A glyph that was drawn by hand keeps the outline of the rasterized glyph**, so its path is no longer the outline of its bitmap: the SVG output draws the face and the bitmap is the printer's, and tracing a hand made cell would put a staircase of a dozen dots in a vector drawing beside the curves of every glyph next to it. The box drawing characters are the exception in the other direction, because they are traced out of the rendered cell of the packed font and are dots to begin with: a hand made glyph inside U+2500 to U+259F moves its box path with it. The two drift on purpose; equality with a refill is not the goal and is not checked here. What the vector output is judged on is the agreement of a whole receipt, which `test/svg.js` measures with a rasterizer.
...
`box` holds the box drawing and block characters, U+2500 to U+259F, which are not outlines at all: they are drawn on the dot grid, stretched to the edges of the cell by the painter, and differ per cell size, so they are traced out of the rendered cell of the packed font, in whole dots. The three cells are the ones the profiles use: 12 by 24 for font A, and 9 by 17 and 9 by 24 for font B. `src/svg/trace.js` is the same trace, which the SVG writer runs over the glyphs a stream downloads into the printer.
