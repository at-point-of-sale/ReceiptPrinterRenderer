...
| `GS ( L pL pH 48 67 ..`, `48 68 ..` | define NV graphics | `a kc1 kc2 b xL xH yL yH [c d..]1..b`, in raster and in column format. The image is kept under its key code for the life of the renderer, an initialize and the end of a stream included. |
...
| `ESC k nL nH d..` | bit image, quadruple density | Section 13. A strip of eight rows, one dot per column, like `ESC L`: a 203 dpi head cannot print more dots per inch, and no specification text settled what it does instead. |
